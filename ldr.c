 ...
 * through successive approxi- mation. The minimum value represents
 * GND and the maximum value represents the voltage on the AREF pin
 * minus 1 LSB. Optionally, AV CC or an internal 2.56V reference
 * voltage may be con- nected to the AREF pin by writing to the REFSn
 ...
 * to the ADC. The ADC is enabled by sett ing the ADC Enable bit, ADEN
 * in ADCSRA. Volt- age reference and input channel se lections will
 ...
 * is in prog ress, will terminate this conversion.
 *
 ...
 * mode. In this mode, the ADC sam- ples and updates the Data Regi
 * sters continuously. Clearing th is bit (zero) will terminate Free
 * Running mode.
 *  
 ...
 * the ADC Conversion Complete Inter- rupt is activated.
 *
 * ------------------------------
